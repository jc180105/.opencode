...
// Tratamento de erro do servidor HTTP (Node.js best practices - Context7)
httpServer.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    console.error(`Porta ${state.port} em uso. Tentando porta ${state.port + 1}...`);
...
// Prevenir crash por exceções não capturadas (Node.js best practice)
process.on('uncaughtException', (err) => {
  console.error('Exceção não capturada:', err.message);
  if (httpServer) {
    httpServer.close(() => {
      console.error('Servidor fechado devido a exceção não capturada');
      process.exit(1);
    });
    setTimeout(() => {
      console.error('Forçando saída após exceção não capturada');
      process.exit(1);
    }, 5000);
  } else {
    process.exit(1);
